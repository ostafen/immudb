...
func (s *Ledger) notify(nType NotificationType, mandatory bool, formattedMessage string, args ...interface{}) {
	s.notifyMutex.Lock()
...
func (s *Ledger) GetBetween(ctx context.Context, key []byte, initialTxID uint64, finalTxID uint64) (ValueRef, error) {
	indexer, err := s.getIndexFor(key)
	if err != nil {
...
func (s *Ledger) Get(ctx context.Context, key []byte) (valRef ValueRef, err error) {
	return s.GetWithFilters(ctx, key, IgnoreExpired, IgnoreDeleted)
}

func (s *Ledger) GetWithFilters(ctx context.Context, key []byte, filters ...FilterFn) (valRef ValueRef, err error) {
	index, err := s.getIndexFor(key)
	if err != nil {
		if errors.Is(err, ErrIndexNotFound) {
			return nil, ErrKeyNotFound
		}
...
func (s *Ledger) GetWithPrefix(ctx context.Context, prefix []byte, neq []byte) (key []byte, valRef ValueRef, err error) {
	return s.GetWithPrefixAndFilters(ctx, prefix, neq, IgnoreExpired, IgnoreDeleted)
}

func (s *Ledger) GetWithPrefixAndFilters(ctx context.Context, prefix []byte, neq []byte, filters ...FilterFn) (key []byte, valRef ValueRef, err error) {
	indexer, err := s.getIndexFor(prefix)
	if err != nil {
		if errors.Is(err, ErrIndexNotFound) {
...
func (s *Ledger) History(key []byte, offset uint64, descOrder bool, limit int) (valRefs []ValueRef, hCount uint64, err error) {
	index, err := s.getIndexFor(key)
	if err != nil {
		if errors.Is(err, ErrIndexNotFound) {
...
func (s *Ledger) MultiIndexingEnabled() bool {
	return s.multiIndexing
}

func (s *Ledger) UseTimeFunc(timeFunc TimeFunc) error {
	if timeFunc == nil {
...
func (s *Ledger) NewTxHolderPool(poolSize int, preallocated bool) (TxPool, error) {
...
func (s *Ledger) writeSnapshot(prefix []byte) (*Snapshot, error) {
	index, err := s.getIndexFor(prefix)
	if err != nil {
		return nil, err
	}
...
func (s *Ledger) Snapshot(prefix []byte) (*Snapshot, error) {
	indexer, err := s.getIndexFor(prefix)
	if err != nil {
		return nil, err
	}
...
func (s *Ledger) SnapshotMustIncludeTxID(ctx context.Context, prefix []byte, txID uint64) (*Snapshot, error) {
...
func (s *Ledger) SnapshotMustIncludeTxIDWithRenewalPeriod(ctx context.Context, prefix []byte, txID uint64, renewalPeriod time.Duration) (*Snapshot, error) {
	index, err := s.getIndexFor(prefix)
	if err != nil {
		return nil, err
	}
...
func (s *Ledger) CommittedAlh() (uint64, [sha256.Size]byte) {
...
func (s *Ledger) PrecommittedAlh() (uint64, [sha256.Size]byte) {
	s.commitStateRWMutex.RLock()
	defer s.commitStateRWMutex.RUnlock()
...
func (s *Ledger) syncBinaryLinking() error {
...
func (s *Ledger) WaitForTx(ctx context.Context, txID uint64, allowPrecommitted bool) error {
	s.waiteesMutex.Lock()
...
func (s *Ledger) ForEachIndex(onIndex func(index *index) error) error {
...
func (s *Ledger) FlushIndexes(cleanupPercentage float32, synced bool) error {
	...
	// TODO: handle cleanupPercentage and synced

	// NOTE: this is a temp hack to make tests pass
	if cleanupPercentage < 0 {
		return tbtree.ErrIllegalArguments
	}
	return s.store.indexerManager.Flush()
}
...
func (s *Ledger) ReadOnly() bool {
	return s.readOnly
}

func (s *Ledger) Synced() bool {
	return s.synced
}

func (s *Ledger) MaxActiveTransactions() int {
	return s.maxActiveTransactions
}

func (s *Ledger) MVCCReadSetLimit() int {
	return s.mvccReadSetLimit
}

func (s *Ledger) MaxConcurrency() int {
	return s.maxConcurrency
}

func (s *Ledger) MaxIOConcurrency() int {
	return s.maxIOConcurrency
}

func (s *Ledger) MaxTxEntries() int {
	return s.maxTxEntries
}

func (s *Ledger) MaxKeyLen() int {
	return s.maxKeyLen
}

func (s *Ledger) MaxValueLen() int {
	return s.maxValueLen
}

func (s *Ledger) Size() (uint64, error) {
	var size uint64
...
func (s *Ledger) fetchAllocTx() (*Tx, error) {
...
func (s *Ledger) releaseAllocTx(tx *Tx) {
	s.txPool.Release(tx)
}
...
func (s *Ledger) fetchAnyVLog() (vLodID byte, vLog appendable.Appendable) {
	s.vLogsCond.L.Lock()
...
func (s *Ledger) fetchVLog(vLogID byte) (appendable.Appendable, error) {
	if s.embeddedValues {
		if vLogID > 0 {
...
func (s *Ledger) releaseVLog(vLogID byte) error {
	if s.embeddedValues {
		if vLogID > 0 {
...
func (s *Ledger) appendValuesIntoAnyVLog(entries []*EntrySpec) (offsets []int64, err error) {
...
func (s *Ledger) appendValuesInto(entries []*EntrySpec, app appendable.Appendable) (offsets []int64, err error) {
...
func (s *Ledger) NewWriteOnlyTx(ctx context.Context) (*OngoingTx, error) {
	return newOngoingTx(ctx, s, &TxOptions{Mode: WriteOnlyTx})
}

func (s *Ledger) NewTx(ctx context.Context, opts *TxOptions) (*OngoingTx, error) {
	return newOngoingTx(ctx, s, opts)
}

func (s *Ledger) commit(ctx context.Context, otx *OngoingTx, expectedHeader *TxHeader, skipIntegrityCheck bool, waitForIndexing bool) (*TxHeader, error) {
...
func (s *Ledger) precommit(ctx context.Context, otx *OngoingTx, hdr *TxHeader, skipIntegrityCheck bool) (*TxHeader, error) {
	if otx == nil {
...
func (s *Ledger) LastCommittedTxID() uint64 {
	s.commitStateRWMutex.RLock()
	defer s.commitStateRWMutex.RUnlock()
...
func (s *Ledger) LastPrecommittedTxID() uint64 {
	s.commitStateRWMutex.RLock()
	defer s.commitStateRWMutex.RUnlock()
...
func (s *Ledger) MandatoryMVCCUpToTxID() uint64 {
	s.commitStateRWMutex.RLock()
	defer s.commitStateRWMutex.RUnlock()
...
func (s *Ledger) performPrecommit(tx *Tx, entries []*EntrySpec, ts int64, blTxID uint64) error {
	s.commitStateRWMutex.Lock()
...
func (s *Ledger) DiscardPrecommittedTxsSince(txID uint64) (int, error) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *Ledger) AllowCommitUpto(txID uint64) error {
	s.commitStateRWMutex.Lock()
	defer s.commitStateRWMutex.Unlock()
...
func (s *Ledger) commitAllowedUpTo() uint64 {
	if !s.useExternalCommitAllowance {
		return s.inmemPrecommittedTxID
	}
...
func (s *Ledger) preCommitWith(ctx context.Context, callback func(txID uint64, index KeyIndex) ([]*EntrySpec, []Precondition, error)) (*TxHeader, error) {
	if callback == nil {
	...
	s.store.indexerManager.PauseIndexing()
	defer s.store.indexerManager.ResumeIndexing()
	...
	if otx.hasPreconditions() {
		s.store.indexerManager.ResumeIndexing()
...
func (s *Ledger) DualProofV2(sourceTxHdr, targetTxHdr *TxHeader) (proof *DualProofV2, err error) {
...
func (s *Ledger) DualProof(sourceTxHdr, targetTxHdr *TxHeader) (proof *DualProof, err error) {
...
func (s *Ledger) LinearProof(sourceTxID, targetTxID uint64) (*LinearProof, error) {
...
// the new Merkle Tree
func (s *Ledger) LinearAdvanceProof(sourceTxID, targetTxID uint64, targetBlTxID uint64) (*LinearAdvanceProof, error) {
...
func (s *Ledger) txOffsetAndSize(txID uint64) (int64, int, error) {
	if txID == 0 {
...
func (s *Ledger) ExportTx(txID uint64, allowPrecommitted bool, skipIntegrityCheck bool, tx *Tx) ([]byte, error) {
...
func (s *Ledger) ReplicateTx(ctx context.Context, exportedTx []byte, skipIntegrityCheck bool, waitForIndexing bool) (*TxHeader, error) {
	if len(exportedTx) == 0 {
		return nil, ErrIllegalArguments
	}
...
func (s *Ledger) FirstTxSince(ts time.Time) (*TxHeader, error) {
	left := uint64(1)
...
func (s *Ledger) LastTxUntil(ts time.Time) (*TxHeader, error) {
	left := uint64(1)
	right := s.LastCommittedTxID()
...
func (s *Ledger) appendableReaderForTx(txID uint64, allowPrecommitted bool) (*appendable.Reader, error) {
	s.commitStateRWMutex.Lock()
	defer s.commitStateRWMutex.Unlock()
...
func (s *Ledger) ReadTxAt(txID uint64, tx *Tx) error {
	return s.readTx(txID, false, false, tx)
}

func (s *Ledger) ReadTx(txID uint64, skipIntegrityCheck bool, tx *Tx) error {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *Ledger) readTx(txID uint64, allowPrecommitted bool, skipIntegrityCheck bool, tx *Tx) error {
...
func (s *Ledger) ReadTxHeader(txID uint64, allowPrecommitted bool, skipIntegrityCheck bool) (*TxHeader, error) {
	r, err := s.appendableReaderForTx(txID, allowPrecommitted)
	if err != nil {
		return nil, err
	}
...
func (s *Ledger) ReadTxEntry(txID uint64, key []byte, skipIntegrityCheck bool) (*TxEntry, *TxHeader, error) {
	var ret *TxEntry
...
func (s *Ledger) ReadValue(entry *TxEntry) ([]byte, error) {
...
func (s *Ledger) ValueReaderAt(
	...
	if !s.embeddedValues && vLogID == 0 && expectedLen > 0 {
...
func (s *Ledger) readValueAt(b []byte, off int64, hvalue [sha256.Size]byte, skipIntegrityCheck bool) (n int, err error) {
	vLogID, offset := decodeOffset(off)
	...
	if len(b) > 0 {
		...
		if !foundInCache {
			n, err = s.readValue(vLogID, offset, b)
			if err != nil {
				return n, err
			}

			if err := s.updateValueCache(off, b[:n]); err != nil {
				return 0, err
			}
		}
	}
...
func (s *Ledger) updateValueCache(off int64, v []byte) error {
	if s.vLogCache == nil {
		return nil
	}
...
func (s *Ledger) readValue(vLogID byte, off int64, buf []byte) (int, error) {
...
func (s *Ledger) validateEntries(entries []*EntrySpec) error {
...
func (s *Ledger) validatePreconditions(preconditions []Precondition) error {
...
func (s *Ledger) Close() error {
	...
	err := s.store.indexerManager.CloseLedgerIndexing(s.ID())
	merr.Append(err)
...
func (s *Ledger) wrapAppendableErr(err error, action string) error {
...
func (s *Ledger) readTxOffsetAt(txID uint64, allowPrecommitted bool, index int) (*TxEntry, error) {
	s.mutex.Lock()
	defer s.mutex.Unlock()
...
func (s *Ledger) TruncateUptoTx(minTxID uint64) error {
	/*
...
func (s *Ledger) ID() LedgerID {
	return s.id
}

func (s *Ledger) Path() string {
	return s.path
}
